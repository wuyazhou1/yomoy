...
public class DictDaoImpl extends GenericRepositoryImpl implements DictDao {
    ...
    public Object GetDictZtree(String parentCode) {
        ...
        List list = this.jdbcTemplate.queryForList("select code \"id\",dict_name \"name\",parent_code \"pId\",REMARK \"remark\" from t_dict_type a start with nvl(parent_code,'-1')= :parentCode  connect by prior code=parent_code"  , parent);
        list.addAll(
            this.jdbcTemplate.queryForList(
                "select dict_value \"id\",dict_key \"name\",dict_type_code \"pId\",REMARK \"remark\" from t_dict a where exists ( " +
                        "select * from (select code from t_dict_type a start with code= :parentCode  connect by prior code=parent_code) b " +
                        "where b.code=a.dict_type_code) order by dict_value"  , parent)
        );
        return list;
    }
}
