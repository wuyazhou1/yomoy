...
$(function(){
    ...
    jQuery.validator.addMethod("isTrim", function(value, element) {
    	var reg = /^\s+|\s+$/g;
		return this.optional(element) || !(reg.test(value)); 
	},"首尾有空格");
	...
	//限制输入数字，且小数点保留
	jQuery.validator.addMethod("numberLimit10_4", function(value, element) {
		var reg = /^[0-9]{0,10}([.]{1}[0-9]{1,4}){0,1}$/;
		return this.optional(element) || (reg.test(value)); 
	},"请输入最大10位整数且小数点保留4位");
	
	//限制输入数字，且小数点保留两位
	jQuery.validator.addMethod("numberLimit10_2", function(value, element) {
		var reg = /^[0-9]{0,10}([.]{1}[0-9]{1,2}){0,1}$/;
		return this.optional(element) || (reg.test(value)); 
	},"请输入最大10位整数且小数点保留2位");
	
	//日期校验
	jQuery.validator.addMethod("isDateNo", function(value, element) {
