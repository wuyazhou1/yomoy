...
<head>
    ...
    <script>
        $(function(){
            var roleCode='<%=request.getParameter("code")%>';
            $("#roleCode").val(roleCode);
            $.ajax({
                url : "/AmoskiUser/MemberManage/getMemberImpowerRole",
                type : "post",
                data : {'role':roleCode},
                async : false,
                dataType : "json",
            ...
            $('#RoleNoImpowerUser').multiSelect({
                selectableHeader: "<div style='background: #56A6E5;color: white;border-radius: 4px 4px 0px 0px;'>未授权用户</div>"+"<input type='text' class='search-input' autocomplete='off' placeholder='try \"12\"' style='width: 162px;'>",
                selectionHeader: "<div style='background: #56A6E5;color: white;border-radius: 4px 4px 0px 0px;'>已授权用户</div>"+"<input type='text' class='search-input' autocomplete='off' placeholder='try \"4\"' style='width: 162px;'>",
        ...
        function saveRoleUserRel(){
            ...
            if($("#RoleNoImpowerUser").val()!=null)
                for(var i=0;i<$("#RoleNoImpowerUser").val().length;i++){
                    if(i==0){
                        user=user+$("#RoleNoImpowerUser").val()[i];
                    }else{
                        user=user+","+$("#RoleNoImpowerUser").val()[i];
                    }
                }
            $.ajax({
                url : "/AmoskiUser/RoleManage/setRoleImpowerUser",
                type : "post",
                ...
                success : function(data) {
                    if(data.code!=null){
                        layer.msg(data.message, {
                            ...
                            window.parent["CloseMemberImpowerRole"]();
                        });
                    }
                }
            });
