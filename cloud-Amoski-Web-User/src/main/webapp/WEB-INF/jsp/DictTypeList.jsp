...
<div class="page-container" style="width: 83%;float: left;margin: 0px -100px 0px -10px;padding-top:0px;">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-10" >
        字典类型名称
        <input type="text" class="input-text" style="width: 180px"  id="dictName" name="dictName" maxlength="50">&nbsp;&nbsp;&nbsp;

        <shiro:hasPermission name="res29">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-10" >
				<span class="l">
                    <shiro:hasPermission name="res31">
					<a class="btn btn-secondary radius" onclick="add()">
						<i class="Hui-iconfont">&#xe600;</i>
						新增
					</a>
                    </shiro:hasPermission>
                    <shiro:hasPermission name="res32">
					<a class="btn btn-primary radius" onclick="edit()">
						<i class="Hui-iconfont">&#xe6df;</i>
						修改
					</a>
                    </shiro:hasPermission>
                    <shiro:hasPermission name="res33">
                ...
                <shiro:hasPermission name="res30">
                <span class="r">
...
<script>
    ...
    function removeMemberAll(node){
        ...
        if(data.length>0){
            ...
            layer.confirm('确定删除该用户吗？', {
                ...
                $.ajax({
                    url:"/AmoskiUser/DictType/DeleteDictType",
                    type:"post",
                    data:{"id":idAll},
                    dataType:"json",
                    async:true,
    ...
    function ShouAddMemberCoupon(node){
        EjectWindow=layer.open({
            ...
            area: ['70%', '90%'],
            content: "/AmoskiWebUser/AMOSKI/AddDictType?source="+node.meath+"&meath=CloseAddEjectWindow&getDate=getEjectWindow"
        });
    }
    var table = null;
    ...
    function initTable() {
        ...
        table = $('#example').DataTable({
            ajax : {
                url : "/AmoskiUser/DictType/DictTypeList",
                type : "POST",
                data : function(d) {
