...
@Controller
@Configuration
public class DictTypePageController  extends BaseController {
    ...
    @RequestMapping("/AMOSKI/DictTypeList")
    public String DictTypeList(HttpServletRequest request){
        ...
        log.info("访问DictTypePageController==》DictTypeList"+((endTime-starTime)/1000)+"开始毫秒"+starTime);
        return "DictTypeList";
    }


    @RequestMapping("/AMOSKI/AddDictType")
    public String AddDictType(HttpServletRequest request){
        long starTime = System.currentTimeMillis();
        Object requestShiro = loginNameCheckedService.findRequestShiro(getIpAddr(request));
        request.getSession().setAttribute("shiro",requestShiro);
        long endTime = System.currentTimeMillis();
        log.info("访问DictTypePageController==》AddDictType"+((endTime-starTime)/1000)+"开始毫秒"+starTime);
        return "AddDictType";
    }
}
