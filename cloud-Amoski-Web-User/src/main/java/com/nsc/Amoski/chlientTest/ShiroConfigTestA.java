...
import com.nsc.Amoski.client.ShrioRedisCacheManager;
import com.nsc.Amoski.client.URLPermissionsFilter;
import com.nsc.Amoski.client.UserWebRealm;
import lombok.Data;
...
public class ShiroConfigTestA {
    ...
    public void setDefaultZone(String defaultZone) {
        ShiroConfigTestA.defaultZone = defaultZone;
    }
    ...
    public ShiroFilterFactoryBean shiroFilter(){
        ...
        String ipDiscaty = ShiroConfigTestA.defaultZone.split("/")[2].split(":")[0];
        //为登入跳转页面
        shiroFilterFactoryBean.setLoginUrl("http://"+ipDiscaty+"/AmoskiWebUser/AMOSKI/loginNameUser");
        // 登录成功后要跳转的链接
        shiroFilterFactoryBean.setSuccessUrl("/");
        //未授权界面;
        shiroFilterFactoryBean.setUnauthorizedUrl("http://"+ipDiscaty+"/AmoskiWebUser/AMOSKI/loginNameUser");
        ...
        filterChainDefinitionMap.put("/amoskiwebuser/**", "anon");//vue前端
    ...
    public UserWebRealm ShiroConfig(){
        UserWebRealm userRealm = new UserWebRealm();
    ...
    public SessionDAO sessionDAO() {
        ...
        haps.add(new HostAndPort("122.114.59.76", 7000));
        haps.add(new HostAndPort("122.114.59.76", 7001));
        haps.add(new HostAndPort("122.114.59.76", 7002));
        haps.add(new HostAndPort("122.114.59.76", 7003));
        ...
        RedisWebManager redisManager = new RedisWebManager();
        ...
        logger.info("进入ShiroConfigTestA==》sessionDAO");
        logger.info("-------------------------------------------------redis配置----------------------------------------------------------");
        logger.info("进入ShiroConfigTestA==》sessionDAO==>redisClusterNodes"+StringUtil.redisClusterNodes);
        logger.info("进入ShiroConfigTestA==》sessionDAO==>redisDatabase"+StringUtil.redisDatabase );
        logger.info("进入ShiroConfigTestA==》sessionDAO==>redisTimeOut"+StringUtil.redisTimeOut );
        logger.info("进入ShiroConfigTestA==》sessionDAO==>redisPoolMaxIdle"+StringUtil.redisPoolMaxIdle);
        logger.info("进入ShiroConfigTestA==》sessionDAO==>redisPoolMinIdle"+StringUtil.redisPoolMinIdle);
        logger.info("进入ShiroConfigTestA==》sessionDAO==>redisPoolMaxTotal"+StringUtil.redisPoolMaxTotal );
        logger.info("进入ShiroConfigTestA==》sessionDAO==>redisPoolMaxMaxActive"+StringUtil.redisPoolMaxMaxActive );
        logger.info("进入ShiroConfigTestA==》sessionDAO==>redisClusterNodes"+StringUtil.redisClusterNodes );
        String[] clusterNodes = StringUtil.redisClusterNodes.split(",");
        ...
        GenericObjectPoolConfig.setMaxTotal(Integer.parseInt(StringUtil.redisPoolMaxTotal));
        GenericObjectPoolConfig.setMaxIdle(Integer.parseInt(StringUtil.redisPoolMaxIdle));
        GenericObjectPoolConfig.setMinIdle(Integer.parseInt(StringUtil.redisPoolMinIdle));
        JedisCluster jedisCluster = new JedisCluster(jedisClusterNodes,Integer.parseInt(StringUtil.redisTimeOut),Integer.parseInt(StringUtil.redisPoolMaxMaxActive), GenericObjectPoolConfig);
 ...
 /*   public static void main(String[] args) {
        ...
        *//*haps.add(new HostAndPort("192.168.5.199", 7000));
        ...
        redisManager.setRedisOperator(redisOperator);
        String key = "1111";
        String value = "22222222222222222";
